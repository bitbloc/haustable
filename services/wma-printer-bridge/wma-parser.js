/**
 * Wongnai Merchant App (WMA) ESC/POS Order Parser
 * Decodes raw thermal printer byte streams (TIS-620 / UTF-8) and extracts structured LINE MAN orders.
 */

// Decode TIS-620 (CP874) buffer to Unicode JavaScript String
...
// Strip ESC/POS control sequences from raw buffer
    ...
    while (i < buffer.length) {
        ...
        if (byte === 0x1B) {
            ...
            if (next === 0x40) { // ESC @ (Init)
                ...
                // ESC a n, ESC E n, ESC ! n, ESC t n, ESC d n, ESC - n
                i += 3; continue;
            } else {
                i += 2; continue;
            }
        ...
        if (byte === 0x1C) {
            const next = buffer[i + 1];
            if (next === 0x2E) { // FS . (Cancel Chinese mode)
                i += 2; continue;
            } else if (next === 0x26 || next === 0x43) { // FS C n
                i += 3; continue;
            } else {
                i += 2; continue;
            }
        ...
        // DLE commands (0x10) e.g. kick drawer
...
export function parseWmaOrderText(rawText) {
    ...
    for (let idx = 0; idx < lines.length; idx++) {
        ...
        // Check if line represents an item line (e.g. "1x อเมริกาโน่เย็น 65.00" or "2 ชาไทย 120.-")
        ...
        // Check if line represents an option / modifier (e.g. "- หวานน้อย 25%", "+ เพิ่มช็อตกาแฟ", "[หวาน 50%]")
        if (currentItem) {
            ...
            if (isOptionLine) {
                ...
                // Secondary note or trailing description for the item
    ...
    // Fallback calculation for total if not explicitly found
...
/**
 * Main entrance to decode & parse raw buffer from TCP socket
...
export function decodeAndParseWmaBuffer(rawBuffer) {
    ...
    // Check if buffer is UTF-8 encoded
    let utf8Text = '';
    ...
    // Prefer whichever encoding yielded valid Thai characters
