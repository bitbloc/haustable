...
const MenuCard = ({ item, mode, onAdd, onRemove, qty, t, sideDishes, sideDishEnabled }) => {
    ...
    return (
        <div className={`group bg-paper rounded-rams p-3 border border-[var(--color-rule)] transition-all relative overflow-hidden ${mode === 'list' ? 'flex gap-4' : ''}`}>

            {/* Image & Side Dish "Bento" Block */}
            <div className={`flex flex-col gap-1.5 ${mode === 'list' ? 'w-24 shrink-0' : 'mb-3'}`}>
                {/* Main Steak Image */}
                <div className={`bg-canvas rounded-rams overflow-hidden relative ${mode === 'list' ? 'aspect-square' : 'aspect-[4/3]'}`}>
                    {item.image_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-subInk">
                            <span className="text-xs font-mono uppercase">No Image</span>
                        </div>
                    )}

                    {mode !== 'list' && qty > 0 && (
                        <div className="absolute top-2 right-2 bg-ink text-paper border border-[var(--color-rule)] px-2 py-0.5 rounded-none flex items-center justify-center text-xs font-mono font-bold z-10">
                            x{qty}
                        </div>
                    )}
                    ...
                    {item.is_recommended && (
                        <div className="absolute top-2 left-2 bg-brand text-ink border border-ink text-[10px] font-mono font-bold px-2 py-0.5 rounded-none flex items-center gap-0.5 z-10 uppercase">
                ...
                {sideDishEnabled && sideDishes && sideDishes.length > 0 && (
                    <div className="grid grid-cols-4 gap-1.5 px-0.5">
                        {sideDishes.map((sd) => (
                            <div key={sd.id} className="aspect-square rounded-rams overflow-visible relative group/sd cursor-help">
                                {/* Thumbnail */}
                                <div className="w-full h-full rounded-rams bg-canvas overflow-hidden border border-[var(--color-rule)] transition-transform hover:scale-105 relative z-10">
                                ...
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-[120px] pointer-events-none opacity-0 group-hover/sd:opacity-100 transition-all duration-300 transform translate-y-2 group-hover/sd:translate-y-0 text-center z-50">
                                    <div className="bg-ink backdrop-blur-md text-paper text-[10px] font-mono font-bold px-2 py-1.5 rounded-none border border-[var(--color-rule)] whitespace-nowrap uppercase">
                                        {sd.name}
                                        {/* Arrow */}
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between">
                <div>
                    <div className="flex justify-between items-start">
                        <h3 className="font-bold text-ink truncate leading-snug">{item.name}</h3>
                    </div>
                    {item.description && <p className="text-xs text-subInk line-clamp-2 mt-1">{item.description}</p>}
                </div>

                <div className="flex items-end justify-between mt-2">
                    <span className="font-mono font-bold text-lg text-ink">{item.price}.-</span>
                    ...
                    {hasOptions ? (
                        <div className="relative overflow-visible"> 
                           ...
                           {ripples.map((ripple) => (
                            ...
                            <button
                                ...
                                className="relative overflow-hidden bg-ink text-paper h-8 pl-3 pr-3 rounded-none border border-[var(--color-rule)] text-xs font-mono font-bold active:scale-95 transition-all duration-300 flex items-center gap-0 group-hover:pr-4 hover:bg-paper hover:text-ink hover:border-ink z-10 uppercase"
                                style={{ maxWidth: '100%' }}
                            >
                        ...
                        qty === 0 ? (
                            <button onClick={() => onAdd(item)} className="bg-canvas text-ink border border-[var(--color-rule)] w-8 h-8 rounded-none flex items-center justify-center hover:bg-paper hover:border-ink transition-colors active:scale-90">
                                <Plus size={16} />
                            </button>
                        ) : (
                            <div className="flex items-center gap-2 bg-ink text-paper rounded-none border border-ink px-2 py-1">
                                ...
                                <span className="font-bold font-mono text-xs w-2 text-center">{qty}</span>
