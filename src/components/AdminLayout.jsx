        ...
        <div className="min-h-screen bg-canvas text-ink font-sans flex flex-col md:flex-row transition-colors duration-300">
            ...
            <aside className="w-64 bg-paper border-r border-gray-200 hidden md:flex flex-col p-6 fixed h-full z-50 shadow-sm">
                <h1 className="text-xl font-bold tracking-tight mb-10 text-ink">
                    The Haus <span className="text-brandDark">Workspace</span>
                </h1>
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link key={item.path} to={item.path}>
                                <div className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${isActive ? 'bg-brand text-ink font-bold shadow-md shadow-brand/20 translate-x-1' : 'text-subInk hover:text-ink hover:bg-gray-50'}`}>
                                    <item.icon size={20} className={isActive ? "text-ink" : "text-subInk"} />
